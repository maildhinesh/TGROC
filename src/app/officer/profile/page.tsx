...
export default async function OfficerProfilePage() {
  ...
  const user = await prisma.user.findUnique({
    ...
    include: { profile: true, contactInfo: true, notificationSettings: true },
  });
    ...
    <DashboardLayout>
      <div className="max-w-2xl space-y-6">
        ...
        <div className="flex items-center gap-5 bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
          <div className="w-20 h-20 rounded-full bg-blue-100 text-blue-700 flex items-center justify-center text-3xl font-bold overflow-hidden">
            {session.user.image ? (
              <img src={session.user.image} alt="" className="w-full h-full object-cover" />
            ) : (
              user.profile?.firstName?.[0] ?? "O"
            )}
          </div>
          <div>
